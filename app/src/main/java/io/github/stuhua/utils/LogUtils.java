...
public class LogUtils {
	private static final String DEFAULT_DIRPATH = "/cookoo/platform/log"; // 存放日志文件的所在路径
	...
	private static final String DEFAULT_LOGNAME = "platform_laucher.txt"; // 存放日志的文本名
	...
	private static void performPrint(int level, String tag, String msg) {
		...
		Log.println(level, tag, "IPODA40 " + threadName + " " + lineIndicator
				+ " " + msg);
		// Toast.makeText(SettingApplication.getApplicateContext(),"tag=" + tag
		// + "...threadName=" + threadName+ "...lineIndicator=" + lineIndicator
	...
	public static void print(String msg) {
		StackTraceElement[] stackTraceElements = new Throwable()
				.getStackTrace();
		String[] fileName = stackTraceElements[1].getFileName().split("\\.");
		performPrint(DEFAULT_LEVEL, "A40"+fileName[0], msg);
		if (isWrite) {
			write(msg);
		}
	}
